...
function SubscriptionManagement() {
  ...
  const handleVerifyCancellations = async () => {
    try {
      setVerifying(true);
      setError(null);
      setResult(null);

      const verifyFunction = httpsCallable(functions, 'manualVerifyFailedCancellations');
      ...
      if (response.data.success) {
        ...
        setError(response.data.message || 'Failed to verify cancellations');
      }
    } catch (err) {
      console.error('Error verifying cancellations:', err);
      setError(err.message || 'Failed to verify cancellations');
    } finally {
      setVerifying(false);
    }
  };
  ...
  return (
    <div className="mb-8 overflow-hidden bg-white border border-gray-200 shadow-sm rounded-xl">
      <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
        <h2 className="flex items-center space-x-2 text-xl font-bold text-secondary-900 font-heading">
          <RefreshCw size={20} />
          <span>Subscription Management</span>
        </h2>
        <p className="mt-1 text-sm text-gray-600">
          Verify and retry failed subscription cancellations
        </p>
      </div>
      ...
      <div className="p-6">
        <div className="flex items-start justify-between">
          <div className="flex-1">
            <h3 className="mb-2 text-lg font-semibold text-secondary-900 font-heading">
              Manual Cancellation Verification
            </h3>
            <p className="mb-4 text-sm text-gray-600">
              This will check for subscriptions that failed to cancel with PayFast and retry the cancellation.
              Only subscriptions with status "cancelled" or "pending_cancellation" that have errors will be retried.
            </p>

            {error && (
              <div className="flex items-start p-4 mb-4 space-x-3 border-l-4 border-red-500 bg-red-50 rounded-r-lg">
                <XCircle size={20} className="mt-0.5 text-red-600 flex-shrink-0" />
                <div>
            ...
            {result && (
              <div className="mb-4 space-y-4">
                <div className="p-4 space-y-3 border-l-4 border-green-500 bg-green-50 rounded-r-lg">
                  <div className="flex items-start space-x-3">
                    <CheckCircle size={20} className="mt-0.5 text-green-600 flex-shrink-0" />
                    <div className="flex-1">
                      <p className="font-medium text-green-900">Verification Complete</p>
                      <div className="mt-2 space-y-1 text-sm text-green-700">
                        <p>• Checked: {result.checked} subscription(s)</p>
                        <p>• Retried: {result.retried} subscription(s)</p>
                        <p className="font-semibold text-green-800">✓ Succeeded: {result.succeeded}</p>
                        {result.failed > 0 && (
                          <p className="font-semibold text-accent-700">⚠ Failed: {result.failed}</p>
                        )}
                      </div>
                    </div>
                  </div>
                ...
                {result.users && result.users.length > 0 && (
                  <div className="overflow-hidden bg-white border border-gray-200 rounded-lg">
                    <div className="px-4 py-3 border-b border-gray-200 bg-gray-50">
                      <h4 className="font-semibold text-secondary-900 font-heading">Users Processed</h4>
                    </div>
                    <div className="overflow-x-auto">
                      <table className="w-full">
                        <thead className="bg-gray-50">
                          <tr>
                            <th className="px-4 py-2 text-xs font-medium text-left text-gray-600 uppercase">User</th>
                            <th className="px-4 py-2 text-xs font-medium text-left text-gray-600 uppercase">Plan</th>
                            <th className="px-4 py-2 text-xs font-medium text-left text-gray-600 uppercase">Token</th>
                            <th className="px-4 py-2 text-xs font-medium text-center text-gray-600 uppercase">Status</th>
                          </tr>
                        </thead>
                          ...
                          {result.users.map((user, index) => (
                            <tr key={index} className="hover:bg-gray-50">
                              <td className="px-4 py-3">
                                <div>
                                  <div className="font-medium text-secondary-900">{user.displayName || 'N/A'}</div>
                              ...
                              <td className="px-4 py-3">
                                <code className="text-xs text-gray-600">{user.subscriptionToken.substring(0, 20)}...</code>
                              </td>
                              <td className="px-4 py-3 text-center">
                                {user.status === 'success' ? (
                                  <div className="flex items-center justify-center space-x-1">
                                    <CheckCircle size={16} className="text-green-600" />
                                    <span className="text-sm font-medium text-green-700">Cancelled</span>
                                  </div>
                                ) : (
                                  <div className="flex flex-col items-center space-y-1">
                                    ...
                                    {user.error && (
                                      <details className="text-xs text-left text-red-600">
                                        <summary className="cursor-pointer hover:text-red-700">View Error</summary>
                                        <pre className="p-2 mt-1 overflow-x-auto bg-red-50 rounded">{JSON.stringify(user.error, null, 2)}</pre>
                                      </details>
                                    )}
                                  </div>
                                )}
          ...
          <button
            onClick={handleVerifyCancellations}
            disabled={verifying}
            className="flex items-center px-6 py-3 ml-6 space-x-2 text-white transition-all bg-accent-500 rounded-lg hover:bg-accent-600 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <RefreshCw size={18} className={verifying ? 'animate-spin' : ''} />
            <span className="font-medium">
              {verifying ? 'Verifying...' : 'Verify Cancellations'}
            </span>
          </button>
        </div>

        {result && result.failed > 0 && (
          <div className="flex items-start p-4 mt-4 space-x-3 border-l-4 border-yellow-500 bg-yellow-50 rounded-r-lg">
            <AlertTriangle size={20} className="mt-0.5 text-yellow-600 flex-shrink-0" />
            <div>
              <p className="font-medium text-yellow-900">Some Cancellations Still Failing</p>
              <p className="text-sm text-yellow-700">
                {result.failed} subscription(s) could not be cancelled. Check the Firebase logs or contact PayFast support.
              </p>
            </div>
          </div>
        )}
...
export default function MasterAccountDashboard() {
  ...
  const loadUsageData = async () => {
    try {
      ...
      const result = await getUsage({ month: selectedMonth, year: selectedYear });
  ...
  const exportIndividualInvoice = (subMaster) => {
    ...
    // CVSift brand colors
    const primaryAccent = [249, 115, 22]; // accent-500
    const darkAccent = [234, 88, 12]; // accent-600
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gray-50">
        <div className="text-center">
          <Loader2 size={48} className="mx-auto mb-4 text-accent-500 animate-spin" />
    ...
    return (
      <div className="flex items-center justify-center min-h-screen p-6 bg-gray-50">
        <div className="w-full max-w-md p-8 bg-white border border-red-200 shadow-lg rounded-xl">
          ...
          <h2 className="mb-2 text-2xl font-bold text-center text-secondary-900 font-heading">Error</h2>
          <p className="mb-6 text-center text-gray-600">{error}</p>
          <button
            onClick={() => navigate('/dashboard')}
            className="w-full px-6 py-3 font-semibold text-white transition-all bg-accent-500 rounded-lg hover:bg-accent-600"
          >
            Back to Dashboard
          </button>
        </div>
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="px-6 py-4 mx-auto max-w-7xl">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <h1 className="text-2xl font-bold text-secondary-900 font-heading">Master Account Dashboard</h1>
            </div>
            ...
            <div className="flex items-center space-x-3 flex-wrap gap-2">
              ...
              <button
                onClick={exportToCSV}
                className="flex items-center px-4 py-2 space-x-2 text-white transition-all bg-accent-500 rounded-lg hover:bg-accent-600"
              >
                <Download size={18} />
          ...
          <div className="flex items-center mt-4 space-x-3">
            ...
            <div className="relative">
              <select
                ...
                className="px-4 py-2 pr-10 text-sm font-medium text-secondary-900 bg-white border border-gray-300 rounded-lg appearance-none cursor-pointer hover:border-accent-400 focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-transparent transition-colors"
              >
              ...
              <ChevronDown size={16} className="absolute text-gray-400 pointer-events-none right-3 top-1/2 -translate-y-1/2" />
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 py-8 mx-auto max-w-7xl">
        ...
        <div className="grid grid-cols-1 gap-6 mb-8 md:grid-cols-2 lg:grid-cols-4">
          ...
          {/* Total Revenue */}
          <div className="p-6 bg-white border border-accent-200 shadow-sm rounded-xl ring-2 ring-accent-500 ring-opacity-20">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center justify-center w-12 h-12 bg-accent-100 rounded-lg">
                    ...
                    <React.Fragment key={subMaster.subMasterId}>
                      <tr className="transition-colors hover:bg-gray-50">
                        <td className="px-6 py-4">
                          <div>
                            <div className="font-medium text-secondary-900">{subMaster.displayName}</div>
                            ...
                            <div className="space-y-4">
                              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                                ...
                                <div className="p-4 bg-white border border-gray-200 rounded-lg">
                                  <h4 className="mb-3 text-sm font-semibold text-secondary-900 font-heading">Account Info</h4>
                                  <div className="space-y-2 text-sm">
                                    <div className="flex justify-between">
                                      <span className="text-gray-600">Plan:</span>
                                      <span className="font-medium text-secondary-900 capitalize">{subMaster.plan}</span>
                                    </div>
                                    <div className="flex justify-between">
                                    ...
                                    <div className="flex justify-between">
                                      ...
                                      <span className="font-medium text-secondary-900">{subMaster.allTime.totalCvs}</span>
                                    </div>
                                  </div>
                                </div>
                                ...
                                <div className="p-4 bg-white border border-gray-200 rounded-lg">
                                  <h4 className="mb-3 text-sm font-semibold text-secondary-900 font-heading">Cost Breakdown</h4>
                                  <div className="space-y-2 text-sm">
                                    <div className="flex justify-between">
                                ...
                                {subMaster.currentMonth.apiCalls > 0 && subMaster.currentMonth.apiBreakdown && (
                                  <div className="p-4 bg-white border border-gray-200 rounded-lg">
                                    <h4 className="mb-3 text-sm font-semibold text-secondary-900 font-heading">API Call Breakdown</h4>
                                    <div className="space-y-2 text-sm">
                                      ...
                                      <div className="flex justify-between pt-2 border-t border-gray-200">
                                        <span className="font-semibold text-secondary-900 text-xs">Total API Calls:</span>
                                ...
                                {/* Historical Trend */}
                                <div className="p-4 bg-white border border-gray-200 rounded-lg">
                                  <h4 className="mb-3 text-sm font-semibold text-secondary-900 font-heading">6-Month History</h4>
                                    ...
                                    <div className="space-y-1">
                                      {detailedUsage[subMaster.subMasterId].slice(-6).map((month, idx) => (
                                        <div key={idx} className="flex justify-between text-xs">
                                          ...
                                          <span className="font-medium text-secondary-900">{formatCurrency(month.totalCost)}</span>
                                        </div>
                                      ))}
                                    </div>
                                  ) : (
        ...
        <div className="p-6 mt-8 border border-blue-200 bg-blue-50 rounded-xl">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h3 className="mb-2 font-semibold text-blue-900 font-heading">Pricing Structure</h3>
