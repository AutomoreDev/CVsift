...
export default function Chatbot() {
  ...
  const TypingIndicator = () => (
    <div className="flex justify-start animate-fade-in">
      <div className="flex items-center gap-3 bg-white border border-gray-200 rounded-2xl px-5 py-3.5 shadow-sm">
        <div className="w-8 h-8 bg-gradient-to-br from-secondary-400 to-purple-600 rounded-full flex items-center justify-center flex-shrink-0">
  ...
  if (showUpgradePrompt) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 via-dominant-50 to-secondary-50 flex items-center justify-center p-4">
        <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full p-8 animate-scale-in">
          ...
          <h2 className="text-3xl font-bold text-secondary-900 mb-3 font-heading">
            AI Assistant
          </h2>
          ...
          <div className="bg-gradient-to-br from-accent-50 to-amber-50 border border-accent-200 rounded-xl p-5 mb-6">
            <p className="text-sm text-accent-900 font-semibold mb-4 flex items-center gap-2">
          ...
          <button
            ...
            className="w-full py-4 bg-gradient-to-r from-accent-500 to-accent-600 text-white rounded-xl hover:from-accent-600 hover:to-accent-700 transition-all font-semibold shadow-lg shadow-accent-500/30 hover:shadow-xl hover:scale-[1.02]"
          >
            Upgrade to Pro
          </button>
  ...
  return (
    <div className="h-screen bg-gradient-to-br from-gray-50 via-purple-50 to-accent-50 flex flex-col overflow-hidden">
      {/* Header - Fixed */}
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center gap-3">
                <div className="w-12 h-12 bg-gradient-to-br from-secondary-400 to-purple-600 rounded-2xl flex items-center justify-center shadow-lg">
                  <Bot className="text-white" size={24} />
                </div>
                <div>
                  <h1 className="text-xl font-bold text-secondary-900 font-heading">CVSift Assistant</h1>
                  <p className="text-sm text-gray-600">AI-powered help • Always here to assist</p>
                </div>
              </div>
            </div>
            <div className="flex items-center gap-2 px-3 py-1.5 bg-gradient-to-r from-accent-100 to-amber-100 text-accent-700 rounded-full text-sm shadow-sm">
              <Sparkles size={14} />
            ...
            {messages.map((message, index) => (
              ...
              >
                {message.role === 'assistant' && (
                  <div className="w-8 h-8 bg-gradient-to-br from-secondary-400 to-purple-600 rounded-full flex items-center justify-center flex-shrink-0 mr-3 shadow-md">
                    <Bot className="text-white" size={18} />
                  </div>
                )}
                ...
                >
                  ...
                  <p className={`text-xs mt-2 ${
                    message.role === 'user' ? 'text-accent-100' : 'text-gray-500'
                  }`}>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {suggestedQuestions.map((question, index) => (
                  <button
                    ...
                    className="px-5 py-3.5 bg-white border-2 border-gray-200 rounded-xl hover:border-accent-300 hover:bg-accent-50 text-sm text-gray-700 transition-all text-left hover:shadow-md hover:-translate-y-0.5"
                  >
                    {question}
                  </button>
                ))}
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-5">
          <form onSubmit={handleSendMessage} className="flex gap-3">
            <input
              ...
              className="flex-1 px-5 py-4 border-2 border-gray-300 rounded-2xl focus:ring-2 focus:ring-accent-500 focus:border-accent-500 transition-all placeholder-gray-400 hover:border-gray-400"
              disabled={isLoading}
            />
            <button
              type="submit"
              disabled={!inputMessage.trim() || isLoading}
              className="px-6 py-4 bg-gradient-to-r from-accent-500 to-accent-600 text-white rounded-2xl hover:from-accent-600 hover:to-accent-700 transition-all disabled:from-gray-300 disabled:to-gray-300 disabled:cursor-not-allowed flex items-center gap-2 font-medium shadow-lg shadow-accent-500/30 hover:shadow-xl hover:scale-105 disabled:shadow-none disabled:scale-100"
            >
              <Send size={20} />
