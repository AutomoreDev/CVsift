...
exports.getSubMasterUsage = onCall({region: "us-central1"}, async (request) => {
  try {
    ...
    const now = new Date();
    const requestedYear = request.data?.year || now.getFullYear();
    const requestedMonth = request.data?.month || (now.getMonth() + 1); // 1-12
    ...
    // Get requested month dates (month is 1-12, but Date constructor uses 0-11)
