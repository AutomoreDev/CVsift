...
/**
 * Helper function to log PII access (POPIA/GDPR compliance)
 * Logs when encrypted candidate PII is decrypted and accessed
 */
exports.logPIIAccess = async (userId, teamOwnerId, cvId, cvName, accessType) => {
  return exports.logActivity({
    userId,
    teamOwnerId,
    action: "pii_accessed",
    ...
    resourceName: cvName,
    metadata: {
      description: `PII decrypted and accessed (${accessType})`,
      accessType: accessType, // "view", "match", "export", "api"
      securityEvent: true,
    },
  });
};
