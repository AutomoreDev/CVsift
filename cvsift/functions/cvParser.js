...
const {logCVUpload, logCVDelete, logCVUpdate, logPIIAccess} = require("./activityLogs");
...
exports.getTeamCVs = onCall(async (request) => {
  try {
    ...
    if (memberSnapshot.empty) {
      // Not a team member, assume they're fetching their own CVs as owner
      teamOwnerId = userId;
    } else {
      // Team member fetching owner's CVs
    ...
    const cvs = await Promise.all(cvsSnapshot.docs.map(async (doc) => {
      ...
      if (cvData.metadata && cvData.metadata._encrypted) {
        ...
        await logPIIAccess(userId, teamOwnerId, doc.id, cvData.fileName || "Unknown CV", "api");
      }
      return {
        id: doc.id,
        ...cvData,
...
exports.getTeamCV = onCall(async (request) => {
  try {
    ...
    if (cvData.metadata && cvData.metadata._encrypted) {
      cvData.metadata = await decryptCVMetadata(cvData.metadata);
      // Log PII access for compliance
      await logPIIAccess(userId, teamOwnerId, cvId, cvData.fileName || "Unknown CV", "api");
    }
...
/**
 * Cloud Function: Get single CV for owner (with PII decryption)
 * Used by CV owners to retrieve their own CVs with decrypted PII
 */
exports.getCVById = onCall(async (request) => {
  try {
    const userId = request.auth?.uid;
    ...
    // Verify this CV belongs to the requesting user
    ...
    // Decrypt PII fields before returning (owner access)
    if (cvData.metadata && cvData.metadata._encrypted) {
      cvData.metadata = await decryptCVMetadata(cvData.metadata);
      // Log PII access for compliance
      await logPIIAccess(userId, userId, cvId, cvData.fileName || "Unknown CV", "view");
    }
...
${cvText.substring(0, 50000)}`,
    ...
    // Update Firestore with encrypted parsed data
